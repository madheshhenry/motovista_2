...
try {
    ...
    if (!$customer) {
        throw new Exception("Unauthorized password reset request");
    }

    // Update password and clear OTP
    $updateStmt = $conn->prepare("UPDATE customers SET password = ?, otp = NULL, otp_expiry = NULL WHERE id = ?");
    $updateStmt->execute([$new_password, $customer['id']]);
