...
public class AdminVerifyResetOtpActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_admin_verify_reset_otp);
        ...
        if (email != null) {
            tvInstruction.setText("Enter the 6-digit security code sent to\n" + email);
        }
    }
    ...
    private void verifyOtp(String otp) {
        ...
        apiService.adminVerifyResetOtp(new OtpRequest(email, otp)).enqueue(new Callback<GenericResponse>() {
            @Override
            public void onResponse(Call<GenericResponse> call, Response<GenericResponse> response) {
                setLoading(false);
                if (response.isSuccessful() && response.body() != null) {
                    if (response.body().isSuccess()) {
                        Intent intent = new Intent(AdminVerifyResetOtpActivity.this, AdminResetPasswordActivity.class);
                        intent.putExtra("email", email);
                        intent.putExtra("otp", otp);
                        startActivity(intent);
                    } else {
                        Toast.makeText(AdminVerifyResetOtpActivity.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(AdminVerifyResetOtpActivity.this, "Verification failed", Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(Call<GenericResponse> call, Throwable t) {
                setLoading(false);
                Toast.makeText(AdminVerifyResetOtpActivity.this, "Error: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void resendOtp() {
        setLoading(true);
        ApiService apiService = RetrofitClient.getApiService();
        apiService.adminForgotPassword(new ForgotPasswordRequest(email)).enqueue(new Callback<GenericResponse>() {
            @Override
            public void onResponse(Call<GenericResponse> call, Response<GenericResponse> response) {
                setLoading(false);
                if (response.isSuccessful() && response.body() != null) {
                    Toast.makeText(AdminVerifyResetOtpActivity.this, "New code sent to your email", Toast.LENGTH_SHORT).show();
                }
            }
